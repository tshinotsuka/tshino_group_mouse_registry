...
- **prefix（`ts`/`sk`）と `cohort`（personal/collab）は独立軸**。prefix は採番した人（同時採番の衝突回避＋来歴）、`cohort` は group 内個体か外部共同研究かを表す。group のマウスである限り採番者が誰でも（`ts`/`sk` どちらでも）`cohort=personal`。両者の一致は要求しない（check も検査しない）。
...
### procedures の主な列
- `procedure_id` = `pr-<mouse_id>-<n>`。**`n` は個体ごとの手技通し番号 = 麻酔セッション(=フォーム 1 枚)ごとに +1**。例 `pr-ts290-01` → `pr-ts290-02` は、ts290 を**別の麻酔セッションでもう一度**手技したとき。同一麻酔内の複数部位/操作は**同じ 1 行**（`site2`…）で表し番号は増やさない。別日/別麻酔なら次番号（独立なら `series_id` 空、段階手術なら `series_id` 共有＋`session_no`）。
...
### matings の主な列
- `mating_id` = `mat-<line_id>-<n>`（`line_id` は**産む仔の系統**、`n` は系統内連番）。例 `mat-aqp4ko-1`。
- `sire_id` / `dam_id`: 親の **birth_id**（colony 参照・**sire=M / dam=F**、check が検証）。
- `pair_start_date`、`status`（`paired` / `active` / `littered` / `ended`）、`notes`。
- 産仔が出たら `litters`（`mating_id` で連結）に `litter_dob` / `n_pups` / `n_male` / `n_female` を記録。残す仔の colony 行 `litter_id` をその litter に向ける（colony→litters→matings）。
- 手入力が面倒なら 🐭 Registry の **産仔を登録** が matings＋litters＋colony を自動採番で同時追加する（番号規則は上と同じ）。
...
3. **genotyping**: colony の gt_* に +/- を入力 → status を confirmed に。
   - 系統ごとに入力するなら **データ → フィルタ表示を作成**で `line_id` を絞る（自分専用ビュー・同時編集を邪魔しない。並べ替えで litter ごとにまとめられる）。`genotype` 列は**数式なので直接打たず**、`gt_*` と `genotype_status` を入力する。
4. **実験投入**: dashboard の「次の mouse_id」を採番 → `animals` 行を作成 → **`birth_id` をドロップダウンで選択**（連結）→ `enrollments` に project/role を記入。
   - `animals` は **birth_id を入れると `sex`/`line`/`dob_intake`/`genotype`/`notes` を colony から自動 pull**（XLOOKUP 式）、`age_weeks` は `dob_intake` と `sac_date` から計算。pull 式は **birth_id を使う新規行だけ**に入れる（歴史個体は birth_id 空で静的値なので、式を被せると消える）。
...
| `snapshot.py` | 全タブ→`snapshots/*.csv` → check（commit 前ゲート）→（`--commit`で）git commit | `python scripts/snapshot.py "<DLした.xlsx>" --commit -m "..."` |
...
- **意味**: sire=M/dam=F、series 同一 mouse_id、procedure_date ≥ dob 等。（`cohort` と mouse_id prefix は独立軸ゆえ一致検査はしない → §2）
...
- `registry_to_metadata.py`（registry CSV → project 別 metadata.yaml、`enrollments.project_id` で絞る）: **実装済み**（解析側 repo `in_vivo_water_imaging_brain` に在・registry の `snapshots/*.csv` を入力に読む＝loose coupling。registry repo には置かない）。既定 dry-run・`--write` は空欄のみ充填。`--write` の実データ検証のみ残。
- ~~`snapshot.py`（全タブ→CSV＋check＋git commit を 1 コマンド）~~: **実装済み**（commit 前に check_registry がゲート。`age_weeks` 等 TODAY() 揮発列は自動除外）。
